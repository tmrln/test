<?php
	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			'COMPANIES' => array('CLIENTCOMPANY', '', 'table.gif', 'None'),
			'ARTICLES' => array('ARTICLES', '', 'table.gif', 'None'),
			'INVOICEVALUES' => array('INVOICEVALUES', '', 'table.gif', 'None'),
			'FORECASTVALUES' => array('FORECASTVALUES', '', 'table.gif', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'COMPANIES' => "`COMPANIES`.`CLTNAM` as 'CLTNAM', `COMPANIES`.`GPRKEY` as 'GPRKEY', `COMPANIES`.`GPRNAM` as 'GPRNAM', `COMPANIES`.`GPRSAM` as 'GPRSAM', `COMPANIES`.`GPRSTT` as 'GPRSTT', `COMPANIES`.`KSTNRA` as 'KSTNRA', `COMPANIES`.`KSTNAM` as 'KSTNAM'",
			'ARTICLES' => "`ARTICLES`.`ARTKEY` as 'ARTKEY', `ARTICLES`.`ARTNAM` as 'ARTNAM', `ARTICLES`.`ATYNAM` as 'ATYNAM', `ARTICLES`.`AGRNAM` as 'AGRNAM'",
			'INVOICEVALUES' => "`INVOICEVALUES`.`CLTNAM` as 'CLTNAM', `INVOICEVALUES`.`INV_GPRKEY` as 'INV_GPRKEY', `INVOICEVALUES`.`INV_GPRNAM` as 'INV_GPRNAM', `INVOICEVALUES`.`INV_ARTKEY` as 'INV_ARTKEY', `INVOICEVALUES`.`INV_ARTNAM` as 'INV_ARTNAM', `INVOICEVALUES`.`INVMEN` as 'INVMEN', `INVOICEVALUES`.`INVMON_F` as 'INVMON_F', `INVOICEVALUES`.`INVJAH_F` as 'INVJAH_F'",
			'FORECASTVALUES' => "`FORECASTVALUES`.`CLTNAM` as 'CLTNAM', `FORECASTVALUES`.`FOR_ARTKEY` as 'FOR_ARTKEY', `FORECASTVALUES`.`FOR_GPRKEY` as 'FOR_GPRKEY', `FORECASTVALUES`.`FOR_ADRKEY` as 'FOR_ADRKEY', `FORECASTVALUES`.`FORMEN` as 'FORMEN', `FORECASTVALUES`.`FORMON_F` as 'FORMON_F', `FORECASTVALUES`.`FORJAH_F` as 'FORJAH_F', `FORECASTVALUES`.`KSTNRA` as 'KSTNRA'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		...
		$pkey = array(   
			'COMPANIES' => 'GPRKEY',
			'ARTICLES' => 'ARTKEY',
			'INVOICEVALUES' => 'INV_GPRKEY',
			'FORECASTVALUES' => 'FOR_GPRKEY'
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('FORECASTING_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('FORECASTING_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['FORECASTING_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['FORECASTING_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> FORECASTING</a>
			</div>
